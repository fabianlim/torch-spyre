...
"""
    ...
    def test_divided_add_golden(self):
        ...
        emitted = generate_ktir("ktir_fused_add_0", self._divided({"d1": 32}))
    ...
    def test_one_core_emits_no_tile_id(self):
        ...
        emitted = generate_ktir("ktir_fused_add_0", self._divided({}))
    ...
    def test_two_divided_symbols_read_the_id_as_mixed_radix(self):
        ...
        emitted = generate_ktir("ktir_add_8", self._divided({"d0": 2, "d1": 4}))
...
"""
    ...
    def _sum_specs():
        ...
        def arg(is_input, index, name, size, coords):
            return TensorArg(
                is_input=is_input,
                arg_index=index,
        ...
        return [
            OpSpec(
                op="sum",
                is_reduction=True,
                iteration_space={lanes: (2048, 32), rows: (256, 1)},
                args=[
                    arg(True, 0, "arg0", [32, 256, 64], [stick, rows, lane]),
                    # The reduced axis is still here, as a unit extent at a
                    # constant coordinate: rank 3 in, rank 3 out.
                    arg(False, 1, "buf0", [1, 32, 64], [sympy.Integer(0), stick, lane]),
                ],
                op_info={},
            )
        ]
