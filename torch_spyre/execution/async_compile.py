...
class SpyreAsyncCompile(AsyncCompile):
    ...
    def ktir(
        ...
        #
        # ``frontend_pool_allocation`` is what ``call_kernel`` passes a pool
        # tensor by, so the emitter needs it to open the signature with a matching
        # slot.  Read from config here: the emitter reads no config.
        ktir_text = generate_ktir(
            kernel_name,
            specs,
            bake_addresses=not _spyre_config.bundle_symbolic_args,
            frontend_pool_allocation=_spyre_config.frontend_pool_allocation,
        )
